#!/usr/bin/env sh
# 회전 원판 생존 미니게임 부품(tools/disc-parts.json → tripo-studio-parts.sh)을 웹용으로 줄인다 — warehouse-glb.sh 와 같은 파이프라인 (Studio v3.1, permissive).
#   sh tools/disc-glb.sh <원본 폴더> [id ...]      → public/world/disc/<id>.glb
set -eu
SRC_DIR="${1:?원본 폴더}"; shift; ONLY="$*"
OUT_DIR="$(cd "$(dirname "$0")/.." && pwd)/public/world/disc"
GLTF="${GLTF_TRANSFORM:-npx --yes @gltf-transform/cli}"
mkdir -p "$OUT_DIR"
# id | 목표 삼각형(k) | 텍스처 | 오차(1 = permissive). 기둥은 한 개(가운데) — 6k, 비콘은 가장자리에 여덟 — 2k
TABLE='
disc_hub|6|512|1
disc_beacon|2|256|1
'
