#!/usr/bin/env sh
# 움직이는 플랫폼 미니게임 부품(tools/platform-parts.json → tripo-studio-parts.sh)을 웹용으로 줄인다 — warehouse-glb.sh 와 같은 파이프라인 (Studio v3.1, permissive).
#   sh tools/platform-glb.sh <원본 폴더> [id ...]      → public/world/platform/<id>.glb
set -eu
SRC_DIR="${1:?원본 폴더}"; shift; ONLY="$*"
OUT_DIR="$(cd "$(dirname "$0")/.." && pwd)/public/world/platform"
GLTF="${GLTF_TRANSFORM:-npx --yes @gltf-transform/cli}"
mkdir -p "$OUT_DIR"
# id | 목표 삼각형(k) | 텍스처 | 오차(1 = permissive). 발판은 여러 장 인스턴스 — 4k
TABLE='
hover_pad|4|512|1
pad_beacon|2|256|1
'
