#!/usr/bin/env sh
# SF 복도 맵(2026-08-29 재구성) GLB 부품을 웹용으로 줄인다 (interrogation-glb.sh 와 같은 파이프라인).
...
#         한 번에: sh tools/tripo-studio-parts.sh tools/corridor-sci-parts.json <원본 폴더> --reduce tools/corridor-sci-glb.sh
#   결과: public/world/corridor/<id>.glb — 삼각형 감소 · 텍스처 축소 · EXT_meshopt_compression
#
#   sh tools/corridor-sci-glb.sh <원본 폴더> [id ...]   (원본은 <폴더>/<id>.glb. id 를 주면 그것만 변환)
#   GLTF_TRANSFORM="경로/gltf-transform" 로 캐시된 CLI 를 쓸 수 있다.
#
# ★ 삼각형 예산 = 인스턴스 수 × 삼각형. 문·링은 복도 끝의 초점이라 넉넉히(2개씩) — 8k 로 깎으니 문이 구겨졌다 (2026-08-29). 콘솔 10개는 12k.
#   노멀·거칠기 맵을 corridor.tsx 가 그대로 쓰므로 텍스처도 문·링은 2048².
set -eu
SRC_DIR="${1:?원본 폴더}"
shift
ONLY="$*"
...
TABLE='
sci_bulkhead|60|2048|1
sci_console|12|1024|1
sci_blast_door|90|2048|1
'
