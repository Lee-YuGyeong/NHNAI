 ...
 * │ 파일 자리는 shared/opening.ts 의 OPENING_SOURCES (첫 줄이 who_is_AI).      │
 * └──────────────────────────────────────────────────────────────────────────┘
 *
 * ┌─ 자동재생은 막히는 것이 정상이다 ────────────────────────────────────────┐
 ...
 * │                                                                          │
 * │ 시작은 OPENING_START_SEC 부터다 (지금 0) — 길이를 받아 온 순간 거기로 놓는다.│
 * │                                                                          │
...
export function OpeningVideo({ onDone, remember = true }: OpeningVideoProps) {
   ...
   * 지금은 0 이라 하는 일이 없지만 남긴다 — 앞이 검은 영상으로 돌아가면 상수 하나로 다시 산다.
   * ★ 그때는 heroes.tsx HeroVideo 의 「두 길」도 같이 가져와야 한다: 캐시가 따뜻하면 metadata 가 커밋보다
   *   먼저 와서 이 핸들러가 안 불린다 (2026-09-05 /intro 에서 겪었다).
   */
  const startAt = () => {
    const v = videoRef.current;
    if (!v) return;
  ...
  return (
    <div className="ov" ref={boxRef}>
      ...
      */}
      ...
      >
        {/*
          자리가 셋이다 — R2 의 who_is_AI 를 먼저 걸고, 안 열리면 저장소의 AV1, 그것도 못 풀면
