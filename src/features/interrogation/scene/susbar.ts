 ...
 * 그런 문턱이 없다 — 대신 **격리선에서 거꾸로 잰다**: 몰이 한 번(지목 + 동조 + 가산 상한)이면 닿는
 * 거리에 붉은 칸을 두어, 색이 붉어진 몸은 "다음 몰이에 격리될 수 있다"로 읽힌다.
 *
 * 숫자를 여기 안 적는다 — 걸음(SUSPICION)은 플레이테스트로 움직이는 값이라(2026-09-05 재조정) 적어 두면
 * 곧 거짓말이 된다. 선은 그 상수에서 계산된다.
 */
import { SUSPICION } from '@/world/mp/game-protocol';

export type SusLevel = 'calm' | 'warm' | 'hot';

/** 붉어지는 선 — 격리선에서 몰이 한 번(지목 + 동조 + 가산 상한) 거리 */
