 ...
 * 고를 수 있게").
 *
 *  · CardOffer  — 세 장이 펼쳐지고 **하나**를 고른다. 서버가 1등에게만 보낸다(game_cards.offer) — 봇이 1등이면 봇이 서버 안에서 쥐고 쓴다(runtime 의 botCards);
 *                 사람에게는 아무것도 안 온다. 고르는 시간은 CARD.offerMs 뿐, 안 고르면 그냥 지나간다.
...
/** 쥔 카드 — 오른쪽 아래. 쓸 상대를 고르는 단계가 카드 안에 펼쳐진다 */
export function CardDock({
  ...
  return (
    <div className="ig-carddock" aria-label="내 카드">
      <p className="hd">내 카드 {canUse ? '' : '· 토론 중에만 쓴다'}</p>
      {items.map((item, i) => {
        const info = CARD_INFO[item];
