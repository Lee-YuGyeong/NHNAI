 ...
 *   - 걷기 명령(WASD → 월드 속도)은 trial_walk 로, 밀치기(Space)는 카메라가 보는 방향과 함께 trial_push 로 올린다
...
export function TowerRig({ selfId, body = null, sendWalk, sendPush }: { selfId: string | null; body?: BodyId | null; sendWalk: (x: number, z: number) => void; sendPush: (hx: number, hz: number) => void }) {
  ...
  useFrame((_, delta) => {
    ...
    // 밀치기 — Space. 눌린 순간 한 번, 쿨다운은 로컬에서도 지킨다(서버가 다시 본다)
    if (input.jump && !pushHeld.current) {
    ...
    } else if (!input.jump) pushHeld.current = false;
