...
export function DodgeRig({ sendMove }: { sendMove: (x: number, z: number, y: number, heading: number, anim: AnimState) => void }) {
  ...
  const pos = useRef<{ x: number; z: number }>({ x: DODGE_SPAWN.x, z: DODGE_SPAWN.z });
  const yaw = useRef(0);
  ...
  useEffect(() => {
    pos.current = { x: DODGE_SPAWN.x, z: DODGE_SPAWN.z };
    yaw.current = 0;
  ...
  useFrame((_, delta) => {
    ...
    const changed = s.anim !== anim || Math.abs(s.x - pos.current.x) > 0.001 || Math.abs(s.z - pos.current.z) > 0.001 || Math.abs(s.heading - h) > 0.001 || Number.isNaN(s.x);
    if (changed && now - s.at >= MOVE_THROTTLE_MS) {
      sendMove(pos.current.x, pos.current.z, 0, h, anim);
      s.at = now;
      s.x = pos.current.x;
      s.z = pos.current.z;
      s.heading = h;
