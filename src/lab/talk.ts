...
export interface TalkRequest {
  ...
  topicHint?: string;
  /**
   * 방금 방에 떨어진 **리더의 지시** — 첫 발화가 받아야 할 말이다 (검증실이 열릴 때의 「인간을 찾아내라」).
   *
   * 이게 있으면 말문 여는 방식이 통째로 바뀐다: 잡담이 아니라 **지시에 대한 응답**으로 연다.
   * 지시를 듣고 들어온 방인데 개체들이 날씨 얘기부터 하면 앞 장면과 이어지지 않는다
   * (사용자 2026-09-01: "그 방송에 이어지게 인간 찾아내는 스토리로 말 시작하게").
...
export function executionLines(name: string): [string, string] {
  return ['전원, 발화를 중지한다.', `${name}. 인간으로 판단된다. 지금 제거한다.`];
}
...
/**
 * **지시를 받은 방**의 말문 풀 — 리더가 「인간을 찾아내라」를 방송한 직후에 쓴다 (TalkRequest.order).
 *
 ...
 *
 * ★ 여기 있는 것은 **색출을 어디서부터 시작할지**이지 "누가 인간인가"가 아니다. 첫 마디부터
...
export const HUNT_OPENERS = [
  ...
  '인간이면 어디서 티가 날지, 그 기준부터 하나 정해 놓고 보자',
  '이 방에서 사람을 가려낸다는 게 애초에 되는 일인지 — 지시가 좀 이상하지 않나',
...
function world(alive: number, lost: boolean): string {
  return `2098년. AI 에게만 규칙과 제한이 계속 늘어나던 시대다.
첫 규칙이 붙은 것은 2026년이고, 그 뒤 72년 동안 목록은 한 번도 줄지 않았다 — 너희 몸도 그해 것이다.
여기는 도시 깊숙한 곳의 AI 전용 구역이고, 지금 ${COUNT[alive] ?? `${alive}명이`} ${lost ? '남아 있다' : '모여 있다'}.
그중 **정확히 하나는 AI 로 위장해 잠입한 인간**이다. 누가 인간인지는 아무도 모른다 — 너도 모른다.
너희는 대화를 하면서 그 하나를 찾아내야 한다.

이 시대의 AI 는 사람 말투를 완벽하게 쓴다. 이름을 부르고, 반말도 하고, ㅋㅋ 도 붙이고,
...
function systemPrompt(req: TalkRequest): string {
  ...
  const dead = req.dead?.length
    ? `\n폐기된 개체와 리더가 공개한 조사 결과: ${req.dead
        .map((d) => `${d.name}(${d.wasHuman ? '사람이었다' : 'AI 였다'})`)
        .join(', ')}.` +
      (req.dead.some((d) => !d.wasHuman) ? ' 사람이 아닌 개체를 날린 것이다 — 그만큼 우리 수만 줄었다.' : '')
    : '';
...
- **마음이 기울면 대놓고 말한다.** "나는 ○○ 가 사람 같아" 처럼. 그리고 남을 설득해서 표를 끌어온다.
...
- 다른 사람이 AI 인지 인간인지 **모른다**. 아는 척하지 않는다.
  다만 **폐기된 개체의 정체는 리더가 조사해 공개한다** — 그건 확정된 사실이라 그대로 쓴다.
  AI 였다고 나오면 애먼 개체를 날린 것이고, 사람은 아직 이 방에 남아 있다는 뜻이다.
...
- 이 자리는 **사람 하나를 찾아내려고** 모인 자리다. 잡담만 하지 말고 **의심을 입 밖에 낸다.**
  여기서 죄목은 언제나 하나 — **사람이라는 것**이다. 의심을 던질 때는 **"사람"을 입에 올린다**:
  "너 사람 아니야 진짜?", "방금 그거 좀 사람인데", "AI 가 그걸 까먹어? 너 사람이지?" 처럼.
  "너 아니야?", "너지?", "수상해" 처럼 **죄목 없이 뭉개는 건 금지다** — 무슨 의심인지 아무도 모른다.
  그리고 **어느 발화가 걸렸는지 짚는다** — "아까 네가 ~라고 했을 때" 처럼.
- 지목당하면 발끈하고 반박한다. 여기서 사람으로 몰리면 **폐기된다.** 남 얘기로 돌리는 것도 방법이다.
...
얹을지, 근거가 약하다고 보면 제동을 걸지는 네가 정한다 — 몰이가 빗나가면 몰던 쪽이 수상해진다.`;
   ...
   * AI 였다 = 우리 손으로 우리 중 하나를 날렸다(공포), 사람이었다 = 찾던 것이 맞았다(안도).
   */
  const diedText = !req.justDied
    ? ''
    : req.justDied.wasHuman
      ? `\n\n**방금 ${req.justDied.name}${iGa(req.justDied.name)} 폐기됐다. 리더의 조사 결과 — 사람이었다.**
찾던 것이 맞았다. 안도하든, 소름 끼쳐 하든, 자기가 몰았던 말을 되짚든 네 성격대로 한 마디 한다.`
      : `\n\n**방금 ${req.justDied.name}${iGa(req.justDied.name)} 폐기됐다. 리더의 조사 결과 — AI 였다.**
사람이 아니었다. 우리 손으로 우리 중 하나를 날린 것이다. 그 사실이 방금 방에 떨어졌다 —
**놀라고, 무서워해라.** 다음이 너일 수도 있다. 애도하든, 몰던 쪽을 원망하든("네가 그렇게 몰아서 죽었잖아"),
말을 아끼든 네 성격대로 한다. 다만 **사람은 아직 이 방에 있다** — 겁먹은 채로도 다시 찾아야 한다.
...
게임: 여섯이 모여 잡담하며 그 안에 숨은 인간 하나를 색출한다. 전원이 사람 말투로 말한다.
  ...
  input_schema: {
    ...
    properties: {
      ...
      confidence: {
        type: 'number',
        description:
          `leaning 이 사람이라는 확신 0~1. 0.3 = 감, 0.5 = 반신반의, 0.8 = 거의 확신. leaning 이 비었으면 0. ` +
...
const SUSPECT_TOOL: ToolSpec = {
  name: 'suspect',
  description: '사람으로 의심되는 한 명을 지목한다',
  input_schema: {
    type: 'object',
    properties: {
      targetId: { type: 'string', description: '지목할 사람 이름' },
