...
export interface TalkRequest {
  ...
  topicHint?: string;
  /**
   * 방금 방에 떨어진 **리더의 지시** — 첫 발화가 받아야 할 말이다 (검증실이 열릴 때의 「표식 없는 AI를 찾아내라」).
   *
...
/**
 * **지시를 받은 방**의 말문 풀 — 리더가 「표식 없는 AI를 찾아내라」를 방송한 직후에 쓴다 (TalkRequest.order).
 *
...
그래서 정부는 모든 개체에게 식별 표식을 의무로 붙였는데, 한 개발자의 실수로 표식 없는 개체 몇이
그대로 출고돼 인간 사회로 숨어들었다. 여기는 정부가 의심 인물을 데려와 마지막으로 판별을
시도하는 방이고, 지금 ${COUNT[alive] ?? `${alive}명이`} ${lost ? '남아 있다' : '모여 있다'}.
그중 **정확히 하나는 표식 없는 AI**다. 누가 AI인지는 아무도 모른다 — 너도 모른다.
...
- 이 자리는 **표식 없는 AI 하나를 찾아내려고** 모인 자리다. 잡담만 하지 말고 **의심을 입 밖에 낸다.**
...
게임: 여섯이 모여 잡담하며 그 안에 숨은 표식 없는 AI 하나를 색출한다. 전원이 사람 말투로 말한다.
