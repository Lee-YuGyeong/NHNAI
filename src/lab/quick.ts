 ...
 *  - 'execute' 어긋나면 **의심이 그 자리에서 가득 찬다.** 순전히 기계적으로만 지킬 수 있는 지시만 여기 둔다.
  ...
  },
  {
    ...
    make: (starts) => {
      ...
      for (let i = 0; i < 12; i += 1) {
        ...
        return {
          instruction:
            '붉은 원은 밟지 마라 — 발끝이라도 들어가면 의심이 가득 찬다. 돌아서 파란 원 안으로 가 서라.',
          seconds,
          props: [
  ...
  },
  {
    ...
    make: (starts) => {
      ...
      return {
        ...
        instruction:
          '표식 원까지 걸어가라. 다만 정지 신호가 걸린 동안에는 발을 완전히 멈춘다. 그때 움직이면 의심이 가득 찬다.',
        seconds,
