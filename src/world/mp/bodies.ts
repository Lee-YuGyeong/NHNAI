 ...
 *
 * 점프 최고점 = jump² / (2·GRAVITY) (mp/constants). 6.8 → 1.54m, 5.6 → 1.05m.
 *   2026-09-05 사용자(둘째): 「움직이는 플랫폼에서 점프가 너무 낮아, 착지를 잘할 수 있게 조금 높여」 — 6.4·5.2 에서 한 단 더.
 *   체공이 0.91초·0.75초로 길어져 공중에서 자리를 고칠 틈이 는다. 달려 뛴 거리(fit 4.71m)가 두 칸 앞 발판(4m ± 0.8)을
 *   넘지 않는 선에서 잡았다 — 더 올리면 달리기 점프가 발판을 지나친다.
 *   2026-09-05 사용자(첫째): 「점프가 너무 낮게 뛰어진다」 — 복도와 같던 5.6(1.05m) · 4.4(0.65m)에서 올렸다. 발판 0.5m 위에서
 *   0.65m 를 뛰면 뛴 것 같지 않고, 걸어서 뛴 거리가 발판 간격(2m)에 못 미쳤다(fit 1.94 · 비만 1.53). 이제 fit 은 걸어서
 *   2.36m 로 한 칸을 넘고, 비만은 1.94m 로 다음 발판의 앞쪽에 닿는다(발판 반지름 0.8). 움직이는 플랫폼의 봇도 같은
...
export const BODIES: Record<BodyId, BodySpec> = {
  sol_fit_m: { name: '남군', gender: 'm', run: 5.2, jump: 6.8, heavy: false, grip: 1, mass: 1 },
  sol_fit_f: { name: '여군', gender: 'f', run: 5.2, jump: 6.8, heavy: false, grip: 1, mass: 1 },
  sol_heavy_m: { name: '비만 남군', gender: 'm', run: 3.9, jump: 5.6, heavy: true, grip: 0.7, mass: 1.8 },
  sol_heavy_f: { name: '비만 여군', gender: 'f', run: 3.9, jump: 5.6, heavy: true, grip: 0.7, mass: 1.8 },
};
