...
/** 봉 윗면이 무대에서 뜬 높이(m) — 스치는 순간 발이 이보다 높아야 넘은 것이다. 0.42 였다 — 점프를 낮추면서(BAR_JUMP_K) 같이 내려 여유 창을 지킨다 */
export const BAR_HEIGHT = 0.36;
/** 봉의 두께(m) — 그리기용 */
 ...
 * 체공(기준 몸 0.92초)과 거의 맞물린다: 내려서자마자 다음 봉이다. 스케줄은 서버가 만들지만 θ·ω 가 스냅샷에
 * 그대로 실리므로 비밀이 아니다 — 숨은 값은 여전히 발밑(BAR_GRIP)뿐이다.
 * 램프 상한이 2.2 인 이유: 최악(하한 2.5 에서 감속 중 이륙)에도 스침이 체공 창(기준 몸 0.12~0.62초, BAR_JUMP_K 반영) 안에
 * 남는 상한이 ≈3.2 rad/s² — 그 밑이다. 이보다 급하면 이륙 순간 완벽했던 점프가 공중에서 오답이 된다. 하한 1.2 는
...
/**
 * 이 판만 이륙 속도를 이만큼 줄인다 (2026-09-05 사용자: "점프했다가 착지하는 속도가 꽤 길어 … 점프부터 착지 속도를 줄이자") —
 * 체공은 속도에 비례라 20% 준다(기준 몸 0.92→0.74초 · 건장 몸 1.12→0.90초), 높이는 제곱이라 36% 준다(0.67m · 0.99m).
 * 중력은 그대로다("중력은 그대로여야해") — 줄이는 것은 다리 힘 쪽이고, 봉 높이(BAR_HEIGHT)를 같이 내려 여유 창을 지켰다
 */
export const BAR_JUMP_K = 0.8;
...
/** 봉에 맞으면 이만큼(ms) 누워 있다 — 그동안 봉은 몸 위를 지나간다. 1600 이었다 (2026-09-05 사용자: "0.8초로" — 최고 속도(5.5)에서 일어날 틈이 없었다) */
export const BAR_DOWN_MS = 800;
