...
export type C2SMessage =
   ...
   */
  | { t: 'kick'; id: string }
  /** 물리 미니게임 방에 들어왔다 — 라운드가 없으면 서버가 하나 연다 (worker/src/trial/runtime.ts) */
  | { t: 'trial_join' }
  /**
...
export type S2CMessage =
  ...
  /** 새 라운드가 열렸다 — 조건값은 없다. 트랙 기하 등 공개 상수는 클라가 이미 안다 (mp/constants) */
  | { t: 'trial_round_start'; game: TrialGame; round: number; startAt: number }
