 ...
 * 사람은 서로 밀 수 있다(Space) — 밀린 몸은 발판 위를 미끄러지고, 끝을 넘으면 떨어진다. 끝까지 안 떨어지면 이긴다.
 *
...
export const TOWER_DEMOLISH_EVERY_MS = 2600;
 ...
 * 클라(발 높이 · 그림자)와 워커(몸의 y)가 같은 식을 쓴다
 */
export function slabSurfaceY(idx: number, tx: number, tz: number, x: number, z: number): number {
  const c = slabCenter(idx);
  return TOWER_TOP - (tx * (x - c.x) + tz * (z - c.z));
}
