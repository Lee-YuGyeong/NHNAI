...
export const SUSPICION = {
  accuse: 15,
  /**
   * 동조 · 몰이를 키웠다 (2026-09-05 사용자: "지목이 많은 상대 위주로 의심도가 많이 올라가게"). 혼자 겨누는 것(15)은
   * 그대로고, **둘째 사람부터** 무겁다 — 여럿이 한 사람을 겨누는 것이 이 판에서 제일 강한 신호다.
   * 셋이 한 사람을 겨누면 15 + (12+6) + (12+6) = 51, 예전엔 43 이었다.
   */
  agree: 12,
  mobPer: 6,
  mobCap: 24,
  /**
   * **거듭 거론** — 지목의 말 없이도 이름·번호가 자꾸 오르내리는 사람 (runtime.noteMentions, docs/SUSPICION.md ⑩).
   * 세 번째 거론부터 두 번에 한 번씩 mention 만큼, 한 좌석에 mentionCap 까지. 몇 번 스친 이름으로는 안 오르고
   * (2026-09-05 사용자: "짧게 몇 번 언급됐는데 확 올리진 말고"), 계속 입에 오르내리면 천천히 오른다.
   * 누계(repeatWeight)는 안 탄다 — 이 문은 평평해야 「스친 이름」과 「몰리는 이름」이 갈린다.
   */
  mention: 4,
  mentionCap: 12,
  claimMatch: -12,
  ...
  echo: 8,
  /**
   * 불렀는데 대답 없이 넘긴다 — **첫 번부터** 문다 (2026-09-05 사용자: "대답을 회피하는 상대, 대답을 안 하는 상대
   * 위주로"). 예전엔 첫 번을 봐줬는데(DUCK_FREE 1) 40초 토론에서는 그 한 번이 전부라 회피가 한 번도 안 물렸다.
   * 침묵 자체는 여전히 벌하지 않는다: 근거가 되는 것은 「불렀는데 안 답한 것」뿐이다 (docs/SUSPICION.md ⑦).
   * 거듭하면 repeatWeight 만큼 무거워진다 — 10 · 15 · 20.
   */
  duck: 10,
