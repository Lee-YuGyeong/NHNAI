 ...
 * 여덟 종이 다 꽂혀 있지만 한 판이 실제로 여는 셋은 판이 열릴 때 후보(game-protocol 의 GAME_TEST_POOL)에서 뽑는다 —
 * 낙하 생존 · 움직이는 플랫폼 · 회전 원판 · 무게 중심 다리 · 무너지는 타워 가운데 셋. 정지선 · 색 사냥 · 회전 봉 넘기는 후보에 넣으면 그 자리에서 다시 선다.
 *
...
import { TowerEngine } from '../trial/tower/engine';
...
export const ENGINES: Partial<Record<TrialGame, () => GameEngine>> = {
  ...
  // 무너지는 타워 — 마당에 탑(TOWER_CENTER)이 서고, 자리는 서버가 적분한다 (HallScene 의 TowerStage · TowerRig, 2026-09-05)
  tower: () => new TowerEngine(),
  // 회전 봉 넘기 — 마당에 무대(BAR_CENTER)가 서고, 자리도 점프도 서버가 적분한다 (HallScene 의 BarStage · BarRig). 2026-09-06 검문소 후보에 들었다가 같은 날 빠졌다 — 엔진은 꽂혀 있되 안 뽑힌다
  bar: () => new BarEngine(),
};
